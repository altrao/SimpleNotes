...
import com.simplenotes.Application
...
class NoteServiceTest {
    ...
    @Test
    fun `should throw when creating note with title too long`() {
        val user = userRepository.createUser("user-long-title", "test-password")
        val longTitle = "a".repeat(121)
        assertThrowsNoteException("Note title cannot exceed 120 characters") {
            noteService.createNote(user, longTitle, "content")
        }
    }

    @Test
    fun `should throw when creating note with content too long`() {
        val user = userRepository.createUser("user-long-content", "test-password")
        val longContent = "a".repeat(256)
        assertThrowsNoteException("Note content cannot exceed 255 characters") {
            noteService.createNote(user, "title", longContent)
        }
    }

    @Test
    fun `should accept note with max length title and content`() {
        val user = userRepository.createUser("user-max-length", "test-password")
        val maxTitle = "a".repeat(120)
        val maxContent = "a".repeat(255)
        val note = noteService.createNote(user, maxTitle, maxContent)
        assertNotNull(note)
    }
